...
class Container:
    ...
    # define constructor
    def __init__(self, volume: int, orders: list = None):
        """
        Container constructor.

        :param volume: Volume of the container (m^3).
        """
        self.volume = volume
    ...
    def volume_left(self) -> int:
        ...
        Calculate and return the volume left in the container.
            ...
            :return: Volume left in the container (cm^3) as int.
        """
        total_volume = 0
...
class OrderAggregator:
    ...
    def aggregate_order(self, customer: str, max_items_quantity: int, max_volume: int):
        ...
        for item in self.order_items:
            if item.customer == customer and item.quantity <= max_items_quantity and item.total_volume <= max_volume:
    ...
    def prepare_containers(self, orders: tuple) -> dict:
        ...
        for order in orders:
            if order.total_volume > self.container_volume:
                ...
                continue
            container = None
            for c in containers.get(order.destination, []):
                if c.volume_left >= order.total_volume:
                    container = c
                    break
            if container is None:
                container = Container(self.container_volume)
                if order.destination not in containers:
